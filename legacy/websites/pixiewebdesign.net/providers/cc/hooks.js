...
// Move this to a database.
    users = [
        { name: 'stephen@hok.io', pass: 'stephen23', roles: ['admin']}
    ];

_.each(users, function (n) {
    database.clients[n.name] = { secret: n.pass, roles: n.roles};
});
...
function generateToken(data) {
    ...
    var sha256 = crypto.createHmac("sha256", random + cfg.security.salt + timestamp);
    return sha256.update(data).digest("base64");
}

exports.grantClientToken = function (clientId, clientSecret, cb) {
    console.log("Granting token", clientId, clientSecret);
