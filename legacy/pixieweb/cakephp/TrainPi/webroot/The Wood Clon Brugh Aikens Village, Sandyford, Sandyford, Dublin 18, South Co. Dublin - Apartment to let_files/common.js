...
if(typeof HTML5Support!=='undefined')
{$.html5support();}
...
gaName=gaName[0];var code=event.charCode||event.keyCode;if(!code||(code&&code==13)){if(_gaq&&gaName){_gaq.push(['_trackEvent','mortgage_centre',gaName]);}}});$('.welcome-arrow').click(function(event){event.stopPropagation();$('.welcome-menu').children('li').children('ul').toggleClass('welcome-active');$('.welcome-menu-li').toggleClass('li-active');$('.welcome-menu').find('div').toggleClass('title-active');$('.welcome-menu').find('div').toggleClass('title-inactive');$('.welcome-menu').find('div').toggleClass('menu-active');});$('body').click(function(){$('.welcome-menu').children('li').children('ul').removeClass('welcome-active');$('#welcome_title').removeClass('title-active');$('#welcome_title').removeClass('menu-active');$('.welcome-menu-li').removeClass('li-active');$('#welcome_title').addClass('title-inactive');});var lazyloadOptions={effect:"fadeIn",threshold:200}
lazyloadCarouselOptions=$.extend({},lazyloadOptions,{event:"pre-load"});if($().lazyload)
{$("img.lazy").show().lazyload(lazyloadOptions);$(".smi-other img[data-original].lazy").show().lazyload(lazyloadCarouselOptions);$("#pbxl_carousel img[data-original].lazy").show().lazyload(lazyloadCarouselOptions);}});function validateEmail(emailField)
...
+'?'
+params.join('&');};function scrollSticky(stickyBox,stickyHeight,padding,topOffset,footerHeight,marginTop,marginLeftClass,defaultPosition)
{var position='relative';if($(window).height()>=stickyHeight)
{if($(document).height()-footerHeight-padding<$(window).scrollTop()+stickyHeight)
{var top=$(document).height()
-stickyHeight-footerHeight-padding;$(stickyBox).attr('style','position:absolute; top:'+top+'px;');$(stickyBox).addClass(marginLeftClass);}
else if($(window).scrollTop()+padding>topOffset)
{$(stickyBox).attr('style','position:fixed; top:'+marginTop+';');$(stickyBox).addClass(marginLeftClass);}
else
{if(defaultPosition)
{position=defaultPosition;}
$(stickyBox).attr('style','position:'+position+';');$(stickyBox).removeClass(marginLeftClass);}}}
function selectAllCheckboxes(checkboxesClass)
{$('.'+checkboxesClass).attr('checked',true);}
function selectNoneCheckboxes(checkboxesClass)
{$('.'+checkboxesClass).attr('checked',false);}