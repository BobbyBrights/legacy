...
MultiMaps.toastContainer=document.createElement("div");$(MultiMaps.toastContainer).attr("id","toastContainer");if(MultiMaps.lazyLoad)
{MultiMaps.checkMapInViewport();}
else if(!MultiMaps.lateLoad)
...
{window.popup.hide();}
window.popup=new GMapsPopup(popupData,MultiMaps.map,{offX:2,offY:-10});window.popup.show();MultiMaps.map.panToBounds(window.popup.getBounds(window.popup));}
...
{var popup=new GMapsPopup(PropertyPopupContents,MultiMaps.map,{offX:20,offY:-75});google.maps.event.addListener(marker,'click',function(){MultiMaps.togglePopup(popup);});}}
MultiMaps.togglePopup=function(popup){if(!popup.showing||popup!==MultiMaps.GMPopup){popup.show();MultiMaps.map.panToBounds(popup.getBounds(popup));}else{popup.hide();}}
...
MultiMaps.checkMapInViewport=function()
{if(!MultiMaps.lazyLoaded)
{var el=document.getElementById(MultiMaps.domElement);var rect=el.getBoundingClientRect();var inViewport=rect.bottom>0&&rect.right>0&&rect.left<(window.innerWidth||document.documentElement.clientWidth)&&rect.top<(window.innerHeight||document.documentElement.clientHeight);if(inViewport)
{MultiMaps.lazyLoaded=true;MultiMaps.mapClick();}}}
