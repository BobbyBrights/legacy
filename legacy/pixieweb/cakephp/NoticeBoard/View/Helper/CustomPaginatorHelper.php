 ...
 */
	public function link($title, $url = array(), $options = array()) {
		$options = array_merge(array('model' => null, 'escape' => false), $options);
		$model = $options['model'];
		unset($options['model']);
 ...
 */
	protected function _pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		$check = 'has' . $which;
		$_defaults = array(
			'url' => array(), 'step' => 1, 'escape' => false, 'model' => null,
 ...
 */
	public function numbers($options = array()) {
		...
		$defaults = array(
			...
			'currentClass' => 'active', 'currentTag' => null
		);
		$options += $defaults;
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					...
					if ($currentTag) {
						...
						$out .= $this->Html->tag($tag, "<span>" . $i . "</span>", array('class' => $currentClass));
					}
				} else {
					$out .= $this->Html->tag($tag, $this->link("<span>" . $i . "</span>", array('page' => $i), $options), compact('class'));
				}
				if ($i != $params['pageCount']) {
					$out .= $separator;
				}
