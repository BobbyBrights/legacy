...
function show_recent_posts($a, $c){
	...
	foreach( $recent_posts as $recent ){
		if(is_home()):
			echo '<a href="' . get_permalink($recent["ID"]) . '" title="Look '.esc_attr($recent["post_title"]).'" ><i class="icon-bookmark-empty"></i> ' .   $recent["post_title"].'</a>';
			
		else:
		echo '<article class="'.$type.'">';
		echo '<h2><i class="icon-bookmark-empty"></i> <a href="' . get_permalink($recent["ID"]) . '" title="Look '.esc_attr($recent["post_title"]).'" >' .   $recent["post_title"].'</a></h2>';
