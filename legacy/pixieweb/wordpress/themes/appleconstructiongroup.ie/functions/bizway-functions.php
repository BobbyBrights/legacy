...
function bizway_nav() {
    if (function_exists('wp_nav_menu'))
        wp_nav_menu(array('theme_location' => 'custom_menu', 'container_id' => 'menu', 'menu_class' => 'ddsmoothmenu', 'fallback_cb' => 'bizway_nav_fallback'));
    else
        bizway_nav_fallback();
}
 ...
 * @param type $ih 
 */
function bizway_get_thumbnail($iw, $ih) {
    $permalink = get_permalink();
    $thumb = get_post_thumbnail_id();
    $image = bizway_thumbnail_resize($thumb, '', $iw, $ih, true, 90);
