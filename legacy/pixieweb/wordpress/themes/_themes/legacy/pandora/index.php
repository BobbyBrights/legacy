...
?>
	...
	<div class="container">
		<div class="content content-mini">
			...
			require_once (TEMPLATEPATH . '/functions/index-pagenavigation.php');
			...
			if (pandora_pagenavigation() == 0){
				...
				while ( have_posts()) : the_post() ?> <?php 
					...
					{
						?><div id="post-<?php the_ID() ?>" class="post-newest-list">
					...
					{
						?><div id="post-<?php the_ID() ?>" class="post-gallery-list">
							...
							<div style="padding:0 10px 0 5px;float:left">
							<a href="<?php the_permalink() ?>" title="<?php printf( __('Permalink to %s', 'pandora'), the_title_attribute('echo=0') ) ?>" rel="bookmark"><?php echo substr(get_the_title(),0,20) ?>...</a>
							<br />
							<?php the_time('F jS, Y') ?>
							</div>
						</div><?php
...
<?php get_footer(); ?>
<?php pandora_slider_header_end(); ?><!-- slider -->
