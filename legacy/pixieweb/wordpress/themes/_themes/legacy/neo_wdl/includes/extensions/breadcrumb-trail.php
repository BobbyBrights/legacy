	...
	elseif ( is_archive() ) {
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g:i a', 'neo_wdl' ) );

			elseif ( get_query_var( 'minute' ) )
				$trail['trail_end'] = sprintf( __( 'Minute %1$s', 'neo_wdl' ), get_the_time( __( 'i', 'neo_wdl' ) ) );

			elseif ( get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g a', 'neo_wdl' ) );
		}

		elseif ( is_date() ) {

			if ( is_day() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( __( 'Y', 'neo_wdl' ) ) ) . '" title="' . get_the_time( __( 'Y', 'neo_wdl' ) ) . '">' . get_the_time( __( 'Y', 'neo_wdl' ) ) . '</a>';
				$trail[] = '<a href="' . get_month_link( get_the_time( __( 'Y', 'neo_wdl' ) ), get_the_time( __( 'm', 'neo_wdl' ) ) ) . '" title="' . get_the_time( __( 'F', 'neo_wdl' ) ) . '">' . get_the_time( __( 'F', 'neo_wdl' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'j', 'neo_wdl' ) );
			}

			elseif ( get_query_var( 'w' ) ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( __( 'Y', 'neo_wdl' ) ) ) . '" title="' . get_the_time( __( 'Y', 'neo_wdl' ) ) . '">' . get_the_time( __( 'Y', 'neo_wdl' ) ) . '</a>';
				$trail['trail_end'] = sprintf( __( 'Week %1$s', 'hybrid' ), get_the_time( __( 'W', 'neo_wdl' ) ) );
			}

			elseif ( is_month() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( __( 'Y', 'neo_wdl' ) ) ) . '" title="' . get_the_time( __( 'Y', 'neo_wdl' ) ) . '">' . get_the_time( __( 'Y', 'neo_wdl' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'F', 'neo_wdl' ) );
			}

			elseif ( is_year() ) {
				$trail['trail_end'] = get_the_time( __( 'Y', 'neo_wdl' ) );
			}
		}
	}

	elseif ( is_search() )
		$trail['trail_end'] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'neo_wdl' ), esc_attr( get_search_query() ) );

	elseif ( is_404() )
		$trail['trail_end'] = __( '404 Not Found', 'neo_wdl' );
