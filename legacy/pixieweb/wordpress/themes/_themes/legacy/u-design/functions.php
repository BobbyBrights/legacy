...
function my_init_styles() {
	...
	if( $udesign_options['google_web_fonts_assoc'] ) {
	    ...
	    if( $google_fonts_string )
		    printf("<link rel='stylesheet' href='http://fonts.googleapis.com/css?family=%s' type='text/css' />\r\n", str_replace(' ', '+', $google_fonts_string));
	}
...
function my_init_scripts() {
	...
	if( $udesign_options['enable_cufon'] ) {
	    ...
	    if ( $udesign_options['cufon_fonts_assoc']['title_headings_font_family'] == 'Comfortaa' )
	...
	wp_enqueue_script('hoverIntent', get_bloginfo('template_url')."/scripts/superfish-1.4.8/js/superfish.combined.js", array('jquery'), '1.0.0', false);
...
<?php		    printf(__('<span class="time">%1$s</span> on <a href="#comment-%2$s" title="">%3$s</a>', 'udesign'), get_comment_time(__('g:i a')), get_comment_ID(), get_comment_date(__('F j, Y')) );
 ...
 * @return string HTML formatted image linking to the Post with $post_id
 */
function customized_featured_image( $post_id, $img_src, $width, $height, $image_alignment = 'alignleft' ) {
    $html = '<span class="custom-frame '.$image_alignment.'"><a href="'.get_permalink( $post_id ).'" title="' . esc_attr( get_post_field( 'post_title', $post_id ) ) . '"><img src="'.get_bloginfo("template_directory").'/scripts/timthumb.php?src='.$img_src.'&amp;w='.$width.'&amp;h='.$height.'&amp;zc=1&amp;q=90" alt="' . esc_attr( get_post_field( 'post_title', $post_id ) ) . '" /></a></span>';
    if( $image_alignment == 'aligncenter' ) $html = '<div style="text-align:center;">'.$html.'</div>';
    return $html;
}
...
// if the WP-prettyPhoto plugin is not active then add the 'wp-prettyPhoto' rel attribute to the default WP gallery links
